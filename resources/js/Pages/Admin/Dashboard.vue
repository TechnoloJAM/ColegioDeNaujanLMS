...
import { 
    Users, BookOpen, Clock, AlertTriangle, ShieldCheck, TrendingUp
...
const selectedRange = ref(props.currentRange || '6_months');
...
// Extremely Compact Line Chart Configuration
    ...
    datasets: [
        {
            ...
            pointRadius: 2, // Tiny points
            pointHoverRadius: 4,
            borderWidth: 1.5, // Ultra-thin line
            fill: true,
            tension: 0.4, 
...
const lineChartOptions = {
    ...
    plugins: {
        ...
        tooltip: {
            ...
            padding: 6, // Micro padding
            titleFont: { size: 9, family: 'Inter' },
            bodyFont: { size: 10, weight: 'bold', family: 'Inter' },
            displayColors: false,
            callbacks: {
                label: function(context) { return `${context.parsed.y} Enrolls`; }
            }
        }
    },
    scales: {
        y: { 
            beginAtZero: true, 
            ticks: { precision: 0, color: '#94a3b8', font: { size: 8 } },
            grid: { color: '#f1f5f9', drawBorder: false }
        },
        x: { 
            ticks: { 
                color: '#94a3b8',
                font: { size: 7 }, // Micro text
                maxTicksLimit: 7 
            }, 
        ...
        <div class="max-w-screen-2xl mx-auto space-y-2 sm:space-y-4 -mt-3">
            ...
            <div class="flex justify-between items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2 px-2 sm:px-0">
                <h1 class="text-sm sm:text-lg font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-1.5">
                    <ShieldCheck class="w-4 h-4 text-blue-600" /> Admin
                </h1>
            ...
            <!-- MICRO METRICS GRID -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-3 px-2 sm:px-0">
                ...
                <div class="bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-sm sm:text-base font-black text-slate-900 dark:text-white leading-none truncate">
                            {{ stats.activeUsers }} <span class="text-[8px] text-slate-400 font-bold">/ {{ stats.totalUsers }}</span>
                        </h3>
                ...
                <div class="bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-sm sm:text-base font-black text-slate-900 dark:text-white leading-none truncate">
                            {{ stats.activeCourses }} <span class="text-[8px] text-slate-400 font-bold">/ {{ stats.totalCourses }}</span>
                        </h3>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-sm sm:text-base font-black text-slate-900 dark:text-white leading-none">{{ stats.totalEnrollments }}</h3>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
                    <div class="min-w-0">
                        ...
                        <h3 class="text-sm sm:text-base font-black text-slate-900 dark:text-white leading-none">{{ stats.pendingEnrollments }}</h3>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-2 sm:gap-4 px-2 sm:px-0">
                ...
                <div class="lg:col-span-2 bg-white dark:bg-slate-800 p-2 sm:p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col">
                    <div class="flex justify-between items-center mb-1">
                        <h3 class="text-[9px] sm:text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tight">Growth Trend</h3>
                        
                        <!-- Micro Select -->
                        <select v-model="selectedRange" @change="filterData" class="text-[7px] sm:text-[8px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded shadow-sm focus:ring-0 cursor-pointer py-0.5 pl-1.5 pr-5 h-5">
                    ...
                    <!-- Extremely short height for mobile -->
                    <div class="flex-1 min-h-[120px] sm:min-h-[160px] w-full">
