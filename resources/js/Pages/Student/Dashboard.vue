...
const activeAnnouncements = computed(() => {
    ...
    return props.announcements.filter(post => {
        if (post.created_at) {
...
// Only show the 3 newest on the dashboard to save space
const previewAnnouncements = computed(() => {
...
const openAllAnnouncements = () => {
    isAllAnnouncementsModalOpen.value = true;
};

const closeAllAnnouncements = () => {
    isAllAnnouncementsModalOpen.value = false;
};
</script>
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl p-3 md:p-4 text-white shadow-sm mb-4 flex justify-between items-center gap-3 relative overflow-hidden">
            ...
            <div class="flex gap-2 z-10 shrink-0">
                <div class="bg-black/20 backdrop-blur-sm px-2.5 py-1 rounded-lg flex items-center gap-1.5 border border-white/10">
                    ...
                    <span class="text-xs font-black">{{ stats.courses || 0 }}</span>
                </div>
                <div class="bg-white text-blue-700 px-2.5 py-1 rounded-lg flex items-center gap-1.5 shadow-sm">
                    ...
                    <span class="text-xs font-black">{{ stats.pending || 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 items-start">
            ...
            <div class="md:col-span-1 lg:col-span-2 space-y-3 md:space-y-4">
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                    ...
                    <div v-if="upcoming && upcoming.length > 0" class="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            <div class="space-y-3 md:space-y-4">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                    <div class="px-3 py-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
                        <h3 class="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest flex items-center gap-1.5">
                            <svg class="w-3 h-3 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"></path></svg>
                            Fresh Updates
                        </h3>
                        <button v-if="activeAnnouncements.length > 3" @click="openAllAnnouncements" class="text-[9px] text-blue-600 font-bold hover:underline">
                            View All ({{ activeAnnouncements.length }})
                        </button>
                    </div>
                    
                    <div v-if="previewAnnouncements.length > 0" class="divide-y divide-slate-100 dark:divide-slate-700">
                        <Link v-for="post in previewAnnouncements" :key="post.id" :href="route('student.courses.show', post.course_id)" 
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900 shrink-0">
                    <h2 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                        <BellRing class="w-4 h-4 text-purple-500"/>
                        Recent Updates (7 Days)
                    </h2>
                    <button @click="closeAllAnnouncements" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition">
                        <X class="w-4 h-4"/>
                    </button>
                </div>

                <div class="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-100 dark:divide-slate-700">
                    <Link v-for="post in activeAnnouncements" :key="post.id" :href="route('student.courses.show', post.course_id)" 
                          @click="closeAllAnnouncements"
                          class="flex flex-col p-4 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition group">
                        <div class="flex items-center justify-between gap-2 mb-1">
                            <span class="text-[8px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest bg-purple-50 dark:bg-purple-900/30 px-2 py-0.5 rounded border border-purple-100 dark:border-purple-800/30">
                                {{ post.course?.title }}
                            </span>
                            <span class="text-[9px] font-bold text-slate-400">{{ new Date(post.created_at).toLocaleDateString() }}</span>
                        </div>
                        <h4 class="text-sm font-black text-slate-900 dark:text-white leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition">{{ post.title }}</h4>
                        <p class="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mt-1 leading-relaxed">{{ post.content.replace(/<\/?[^>]+(>|$)/g, "") }}</p>
                        <p class="text-[9px] font-bold text-slate-400 mt-2 uppercase tracking-widest">Posted by: {{ post.user?.name }}</p>
                    </Link>
                </div>
            </div>
        </Modal>
